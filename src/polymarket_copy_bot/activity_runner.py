...
class MarketActivityTracker:
    ...
    def get_copy_buy_usd(self) -> float:
        ...
        if balance < MIN_COPY_USD:
            self._debug("Insufficient collateral", {"balance": balance})
            return 0.0
        amount = max(MIN_COPY_USD, balance * BASE_BALANCE_FRACTION)
    ...
    def repeat_trade(self, trade: ActivityTrade, copy_buy_usd: float) -> dict:
        if copy_buy_usd <= 0:
            return {"ok": False, "message": f"skipped: insufficient collateral for min ${MIN_COPY_USD:.2f} copy size"}
